...
send_mail_submit.addEventListener("click", async function () {
    ...
    let data = await axios.post("https://pepcalender.herokuapp.com/mail/to", { body, email, subject })
    send_mail_popup.style.display = "none"
})

remind_faculty_submit.addEventListener("click",async function(){
    let data = await axios.post("https://pepcalender.herokuapp.com/mail/remind")
